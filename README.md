...
Опять-таки некоторое время поэкспериментировав с самостоятельной настройкой маршрутизации - решил использовать готовый образ `nat-instance`. 
...
# Следующий сайт:
```
К сожалению, выяснилось, что сервисы нужно ещё и заставить работать по `https`... 
...
Ради ускорения процесса снова взял готовую роль из ansible-galaxy. И снова пожалел. "Из коробки" ничего не заработало. Т.е. стало работать ... но без репликацции. Отрезал побольше лишнего, что мешало пониманию своей громоздкостью (к примеру, варианты для разных архитектур), добился простоты, понятной для меня. 
Перекомпоновал таски понятным и правильным образом, используя `mysql_replication`:
...
21.09.2022: Разобрался с Wordpress. Раз все настройки в базе - то единожды настроил, перевёл на https, затем сохраниkл дамп MySQL. Ansible разворачивает дамп. Результат:
...
Судя по тому, что все трое в логах сообщают, что `SSL routines:ssl3_get_record:wrong version number) while SSL handshaking to upstream`, на них реально работает http, а не https.
  Первой исправляю grafana и начинаю понимать, что будет странно везде заново выпускать сертификаты. Если будет время, то переделаю на правильный вариант:
...
Alertmanager - на его запуск c https ушло чрезмерное количество времени... Хотя всё-таки заработал:
...
В ходе работы с дипломом я смог совершить типовую ошибку, выложив секретные ключи от AWS (они не имели отношения к диплому, но были в комментариях, т.к. я использовал свой старый tf файл) и от Terraform Cloud, приобретя неоценимый опыт ))
  Т.е. увидел, как оперативно мониторят гитхаб заинтересованные стороны, как в AWS поддержка изящно блокирует доступ для невладельца, наложив "Quarantine Policy" на пользователя (т.е. типовым способо, добавлением политики, доступным для управления мной, при желании), познакомился с сервисом GitGardian. Думаю, что единожды не сделав ошибку - я не был бы так осторожен, как после этого ))
