...
Использован альтернативный вариант. https://i.vgy.me/4HtXdY.png
...
Сначала я оптимистично перенаправил все входы на конкретные порты определённых доменов. Но позже к этому правильно отнёсся только Gitlab. Очень жаль, т.к. намечалась красивая единообразная схема проксирования "сайт: домен, апстрим (адрес,порт)", типа:
```yml
...
Prometheus: "Prometheus supports basic authentication and TLS. This is **experimental** and might change in the future." ( https://prometheus.io/docs/prometheus/latest/configuration/https/ ). Ладно, сказано на https, значит на https...
 C Prometheus, как и с многими другими теперь связана приключенческая история. Сначала он не хотел принимать в prometheus.service опцию `--web.config.file`, выяснилось, что в роли была прописана версия, которая ещё не умела работать по https.  Затем неуспехом заканчивалась проверка `promtool check config web.yml`, пока я таки не перечитал внимательно документацию и не понял, что надо указывать тип конфига, т.е. `promtool check web-config web.yml`.

 Итак: Prometheus работает: https://i.vgy.me/fjKqlF.png 

 Grafana связана с ним: https://i.vgy.me/ebuo8K.png
 И получает данные от node_exporter-ов: https://i.vgy.me/tBKs0s.png (в момент скриншота работали 3 сервера (из экономии) - они видны в дашборде).

Alertmanager заработал 1 раз случайно по https, но я не зафиксировал этот момент, после этого не смог воспроизвести ситуацию. Из хорошего - он работает, шлёт мне в Slack сообщения (я искусственно провоцировал проблемы, условиями типа: волноваться, когда менее 85% диска свободно):  https://i.vgy.me/w58h6Q.png

Сейчас (16.09.2022) на запуск alertmanager-а c https ушло чрезмерное количество времени... Хотя всё-таки заработал: https://i.vgy.me/3rOfQO.png
Grafana видит данные из alertmanager: https://i.vgy.me/cAn6hu.png
