...
Минусы: оперативно решая ошибку я просто скопировал файлы репозитория в сторону и уничтожил репозиторий (вместе с историей). Не очень-то была нужна эта история, но на будущее всё равно опыт - надо разобраться с вопросами зачистки данных в старых коммитах. 
  Поэкспериментировал с https://rtyley.github.io/bfg-repo-cleaner/ но это было с черновиками, не с чистовиком. Но в целом должно быть удообно, вносим все известные секреты в файл, к примеру `passwords.txt`и натравливаем BFG на локальный репозиторий: `bfg --replace-text passwords.txt  my-repo.git`. Вместо секретов появляется `***REMOVED***`.  
